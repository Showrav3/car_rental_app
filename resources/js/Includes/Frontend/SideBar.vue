...
<template>
    <div class="sidebar1 pe-4 pb-3">
        <nav class="navbar navbar-secondary">
            <Link :href="route('show.home')" class="navbar-brand mx-4 mb-3">
            ...
            <div class="d-flex align-items-center ms-4 mb-4">
                ...
                <div class="ms-3" v-if="authUser">
                    <h6 class="mb-0 text-light">{{ authUser.name }}</h6>
                    <span>{{ authUser.role }}</span>
                </div>
            </div>
            <div class="navbar-nav w-100">
                <Link :href="route('show.user.profile', {id: authUser.id})" class="nav-item nav-link" :class="{'active': $page.url === `/user/profile/${authUser.id}`}"><i class="fa fa-user" style="margin-right: 5px;"></i>Profile</Link>
